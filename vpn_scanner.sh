...
# ────────────────────────[ Start Scanning ]───────────────────────────────────
echo "Starting VPN scan..."
if [ "$site_choice" -eq 1 ]; then
...
read -p "Do you want to resolve domains to IP addresses? (y/n) " -n 1 -r
echo
[[ ! $REPLY =~ ^[Yy]$ ]] && echo "Exiting without DNS resolution" && exit 0
...
while true; do
    if [ -s "$ip_file" ]; then
        read -p "Do you want to ping all resolved IPs? (y/n) " -n 1 -r
        echo
        [[ ! $REPLY =~ ^[Yy]$ ]] && break
